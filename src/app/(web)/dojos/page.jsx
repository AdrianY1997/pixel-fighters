      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 px-8">
        {Dojoslist &&
          Dojoslist.map((e) => (
            <div
              key={e.category_id}
              className="bg-gradient-to-r from-white-400 to-gray-200 rounded-lg p-4 border-solid border-2 border-gray-50 shadow-md"
            >
              <div className="flex flex-row">
                <div className="bg-black w-20 h-20 rounded-md m-2"></div>
                <h1 className="text-lg font-bold mb-2">
                  {e.category_name[0].toLocaleUpperCase() +
                    e.category_name.slice(1)}
                </h1>
              </div>
              <div className="flex flex-row">
                <div className="flex flex-col flex-grow">
                  <p className="text-gray-600 flex-grow h-36 overflow-hidden">
